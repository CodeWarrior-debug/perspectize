...
function renderIngestion(): HTMLElement {
  ...
  return section('2 · Ingestion & enrichment', 'Where do the values come from, and what makes two rows the same row?', [
    ...
    grid2([
      field(
        'Link hosts for auto-detect',
        input((d.detect?.hosts ?? []).join(', '), (v) => {
        ...
        'Comma-separated. Leave blank and the Add Content card never guesses this type.'
      ),
      field(
        'Link path pattern',
        input(d.detect?.path ?? '', (v) => {
          d.detect = { hosts: d.detect?.hosts ?? [], path: v, examples: d.detect?.examples ?? [] };
        }, '^/art/collection/search/(\\d+)/?$'),
        'Regular expression over the URL path. Group 1 is the external id. Try it in section 6.'
      )
    ]),
...
function renderSamples(current: DraftState, grid: GridPreview): HTMLElement | null {
  ...
  for (const id of current.selected) rows.push(...previewRowsFor(id, current, fullness));
  if (rows.length === 0) return null;

  const valueOf = (row: PreviewRow, colId: string): string | undefined => {
  ...
  const body = rows.map((row) => {
    const tr = el(
      ...
      visible.map((rc) => {
        ...
        if (rc.col.id === 'type') return el('td', { class: 'muted' }, [typeLabel(row.typeId, current.draft)]);
        const binding = bindingFor(rc.col, row.typeId, current);
        if (!binding) return el('td', { class: 'gap', title: `Not bound for this type — renders "${rc.col.gapFallback}"` }, [gapText(rc.col)]);
    ...
    if (focus && focus.typeId === row.typeId && focus.index === row.index) tr.classList.add('focused');
    return tr;
  });

  const modes: [Fullness, string][] = [
    ['full', 'Full'],
    ['usual', 'Usual'],
    ['minimum', 'Minimum']
  ];
...
function thumbBox(typeId: string, cell: SampleCell | undefined, size: 'cell' | 'modal', contain: boolean): HTMLElement {
...
function itemCell(row: PreviewRow, pc: PreviewCell, tooltip: string, appearance?: string): HTMLElement {
...
function stateCell(pc: PreviewCell, tooltip: string, label: string, applicability: Applicability): HTMLElement {
  ...
  if (pc.state === 'placeholder') td.classList.add('ph');
  return td;
}

function focusRow(row: PreviewRow, target: 'details' | 'perspective'): void {
...
function focusedRow(): PreviewRow | null {
...
function mockValue(pc: PreviewCell | undefined): HTMLElement {
  if (!pc || pc.state === 'allowed-empty') return el('span', { class: 'm-empty', title: 'Empty is allowed' }, ['—']);
  if (pc.state === 'missing-required') return el('span', { class: 'm-bad', title: 'Required, but missing' }, ['missing']);
  const cell = pc.cell;
...
function renderDetailsMock(row: PreviewRow): HTMLElement {
  ...
  for (const col of COLUMNS) {
    if (MOCK_SKIP.has(col.id)) continue;
  ...
  const textSection = (colId: string, heading: string): HTMLElement[] => {
...
function renderPerspectiveMock(row: PreviewRow): HTMLElement {
...
function renderSurfaces(): HTMLElement {
  const row = focusedRow();
  const blurb =
    'What a row opens into. Pick a row by clicking its title in the preview above, or choose one here. These are mockups of ActivityDetailsModal.svelte and PerspectivePopover.svelte, drawn from the bindings, not the real components.';
  if (!row) return el('section', { id: 'surfaces' }, [el('h2', {}, ['5 · Row details & perspective']), el('p', { class: 'blurb' }, [blurb]), el('p', { class: 'muted' }, ['Select a type with sample rows in section 4.'])]);
  ...
  const note =
    'The form is the same for every type; only the title line comes from the content. Check that the standard fields (Quality, Agreement, Importance, Confidence) make sense for this type.';
...
function renderAddCard(): HTMLElement {
  ...
  const paint = () => {
    const r: DetectResult = detect(inputEl.value, state);
    ...
    const options: [string, string][] = [
      ...
      ...(d.enumValue ? ([[d.enumValue, `${d.label || 'New type'} (new)`]] as [string, string][]) : [])
    ];
    for (const [v, label] of options) menu.append(el('option', { value: v, selected: v === r.type }, [label]));
    ...
    if (r.type === 'CLAIM') extras.push(el('p', { class: 'm-muted small' }, ["Claims can't be added from here yet. Pick a type above to continue."]));
