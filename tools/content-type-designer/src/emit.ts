...
import { bindingFor, gapText, resolveGrid, samplesFor, typeLabel, unitKeysFor, type DraftState } from './model.js';
...
export function buildSpec(state: DraftState): string {
  ...
  if (mixedSorts.length) {
    ...
    out.push('');
    out.push(
      'These columns carry different units in this selection. Sorting one of them is never blocked. It is applied grouped by unit first, then ordered within each group, so only numbers that share a unit are ever compared:'
    );
    out.push('');
    ...
    out.push(
      table(
        ...
        mixedSorts.map((rc) => [rc.header || rc.col.generic, unitKeysFor(rc).join(' · ')])
      )
    );
  }
