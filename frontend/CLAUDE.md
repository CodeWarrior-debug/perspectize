...
**Fresh cache headers don't cover a page that is already running.** Mobile browsers keep a bookmarked/home-screen tab alive (in memory or the back-forward cache) across deploys, so it is still on the old build when it comes back. Its next lazy import asks for a chunk the new deploy deleted, gets the `_redirects` HTML, and the page goes blank until a manual refresh. `_headers` can't fix this, and `_redirects` can't answer 404 for `/_app/*` (Cloudflare-style `_redirects` only supports 200/3xx). Recovery happens on the client in three layers. Keep all three:
...
- `lib/utils/versionWatch.ts` (mounted in `+layout.svelte`): checks `_app/version.json` when the tab becomes visible or is restored from bfcache, and reloads if a newer build exists. It skips the reload while an input or contenteditable has focus.
